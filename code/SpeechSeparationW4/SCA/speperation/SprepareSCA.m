function [ft_max]=SprepareSCA(s,X,S,S1,S2,Fs1,Fs2,filename1,filename2,upload)
close('7')
...
%     [W1,Emax,fym,Fit] = SCAoptimum(s,X,rt,upload); %�������������㷨
      [W1,Emax,fym,Fit,YY,sepresults,Y] = SCAoptimum(s,X,rt,upload); %�������������㷨
...
audiowrite([upload,'\11SCA\',YY1wav],YY(1,:),Fs1);
audiowrite([upload,'\11SCA\',YY2wav],YY(2,:),Fs2);

PESQ(rt,1) = pesq_cd(filename1,[upload,'\11SCA\',YY1wav]);
PESQ(rt,2) = pesq_cd(filename2,[upload,'\11SCA\',YY2wav]);

PESQ(rt,4) = pesq_cd(filename1,[upload,'\11SCA\',YY2wav]);
PESQ(rt,5) = pesq_cd(filename2,[upload,'\11SCA\',YY1wav]);
% --------------------------Yfinal����ָ��--------------------------
...
audiowrite([upload,'\11SCA\',Y1wav],Y(1,:),Fs1);
audiowrite([upload,'\11SCA\',Y2wav],Y(2,:),Fs2);

PESQY(rt,1) = pesq_cd(filename1,[upload,'\11SCA\',Y1wav]);
PESQY(rt,2) = pesq_cd(filename2,[upload,'\11SCA\',Y2wav]);

PESQY(rt,4) = pesq_cd(filename1,[upload,'\11SCA\',Y2wav]);
PESQY(rt,5) = pesq_cd(filename2,[upload,'\11SCA\',Y1wav]);
% ------------------PESQ------------------
