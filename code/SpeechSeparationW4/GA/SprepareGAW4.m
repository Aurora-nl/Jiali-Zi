function [ft_max]=SprepareGAW4(s,X,S,S1,S2,Fs1,Fs2,filename1,filename2,upload)
% close('7')
%调用粒子群算�?,求解W1
%W1 = Pso(X)
...
for rt = 1:runtime
    ...
    [W1,Emax,fym,Fit,YY,sepresults,Y] = GA_mainfunctionW4(s,X,rt,upload); %调用遗传算法
...
audiowrite([upload,'\4GA\',YY1wav],YY(1,:),Fs1);
audiowrite([upload,'\4GA\',YY2wav],YY(2,:),Fs2);

PESQ(rt,1) = pesq_cd(filename1,[upload,'\4GA\',YY1wav]);
PESQ(rt,2) = pesq_cd(filename2,[upload,'\4GA\',YY2wav]);

PESQ(rt,4) = pesq_cd(filename1,[upload,'\4GA\',YY2wav]);
PESQ(rt,5) = pesq_cd(filename2,[upload,'\4GA\',YY1wav]);
...
audiowrite([upload,'\4GA\',Y1wav],Y(1,:),Fs1);
audiowrite([upload,'\4GA\',Y2wav],Y(2,:),Fs2);

PESQY(rt,1) = pesq_cd(filename1,[upload,'\4GA\',Y1wav]);
PESQY(rt,2) = pesq_cd(filename2,[upload,'\4GA\',Y2wav]);

PESQY(rt,4) = pesq_cd(filename1,[upload,'\4GA\',Y2wav]);
PESQY(rt,5) = pesq_cd(filename2,[upload,'\4GA\',Y1wav]);
% ------------------PESQ------------------
