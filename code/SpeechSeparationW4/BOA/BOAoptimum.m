function [best_pos,Emax,fmin,Fit,Yfinal,sepresults,YYY] = BOAoptimumW4(Ss,XX,rt,upload)
display('BOA');
...
for i=1:n
    Fitness(i)=FitnessFunW4(Sol(i,:),XX);
%     Fitness(i)=FitnessFunW4NOXcorr(Sol(i,:),XX);
end
...
for t=1:N_iter
    ...
    if t==N_iter  
        ...
        for i=1:n % Loop over all butterflies/solutions
          ...
          Fnew(i)=FitnessFunW4(S(i,:),XX);
%           Fnew(i)=FitnessFunW4NOXcorr(S(i,:),XX);
          ...
          if rand<p  
            ...
            W2=[S(i,:);fliplr(S(i,:))];
            Fnew1(i)=FitnessFunW4(S(i,:),XX);  %Fnew represents new fitness values
%             Fnew1(i)=FitnessFunW4NOXcorr(S(i,:),XX);
