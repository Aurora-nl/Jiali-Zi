...
% function [Best_pos,Best_score,curve] = CSAoptimum(fobj,lb,ub,dim,nPop,Max_iteration)
function [W1,Emax,fitness_min,Fit,Yfinal,sepresults,YYY] = CSAoptimum(Ss,XX,rt,upload)
display('CSA');
...
xn=x;
for i = 1:N
%     ft(i)=FitnessFunNew(xn(i,:),XX); % Function for fitness evaluation
ft(i)=FitnessFun(xn(i,:),XX); % Function for fitness evaluation
end
...
for t=1:Max_iteration
    ...
    xn=xnew;
    for i = 1:N
    ft(i)=FitnessFun(xn(i,:),XX);  % Function for fitness evaluation of new solutions
    end
